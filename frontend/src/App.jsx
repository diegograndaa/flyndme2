...
function App() {
  // Orígenes: dos campos creados desde el inicio
  ...
  // Ref para hacer scroll suave al buscador
  const searchRef = useRef(null);

  // Cuando pasamos a la pantalla de búsqueda, hacer scroll suave
  useEffect(() => {
    if (hasStarted && searchRef.current) {
  ...
  // Botón de inicio: activa la app y, si no hay fecha, propone una por defecto
  const loadDemo = () => {
    setHasStarted(true);
    if (!departureDate) {
  ...
  return (
    ...
    >
      {/* HEADER CON LOGO (click para volver a la landing) */}
      ...
      {!hasStarted ? (
        <>
          ...
          {/* (Aquí mantengo todas tus secciones extra de landing tal como las tenías) */}
          {/* ... Cómo funciona, Por qué FlyndMe, Integraciones, Para quién es, Footer ... */}
          {/* Para no hacer el mensaje infinito, si quieres luego las ajustamos una a una. */}

        </>
      ) : (
        <>
          {/* MAIN APP */}
          <main className="py-4" ref={searchRef}>
